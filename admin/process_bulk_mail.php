...
$message = "$message";
 	   ...
 	   if (!$subject){ 
echo("<font color=red><strong>Error:</strong>Please Enter The Subject</font>". '<br>' . '<a href="javascript:history.go(-1);">[Back] </a>');
exit;}
...
exit;}
   ...
   if (!$message){ 
echo("<font color=red><strong>Error:</strong>Please Type The Message</font>". '<br>' . '<a href="javascript:history.go(-1);">[Back] </a>');
exit;}
...
/* Start of headers */
$mailheads= "From:info@wittybits.org";
...
if (file($tmpName)) {
  ...
  $mailheads .= "\nMIME-Version: 1.0\n";
  $mailheads .= "Content-Type: multipart/mixed;\n" ;
  $mailheads .= " boundary=\"{$mimeBoundary}\"";
...
$mailSent = mail($to, $subject, $message, $mailheads);

$ms="<br><div data-alert class='alert-box success'>
						Message has been sent!
                		
                	</div> </strong>";
					
$sql3="Insert into letters(message, month, year, sentday, recipients, totalrecipients) value('$message', '$thisMonth', '$thisYear', '$dbdate', '$recipients', '$total_recipient')";
...
<div id="main_container">
  ...
  <div class="header">
    ...
    <h2> Sent Bulk Mails</h2>
    <div class="large-12 columns"> <strong>Successful! 
      </strong>
      <hr/>
        <?php echo $ms; ?> </div>
    <p>&nbsp;</p>
